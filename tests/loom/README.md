...
This directory owns a standalone Loom model crate for small bounded concurrency checks.
...
| --- | --- | --- | --- |
| `tests/loom/tests/wal_durable_before_visible.rs` | A visible commit LSN is never observed before the corresponding WAL LSN is durable. | Two Loom atomics: `durable_lsn` and `visible_lsn`. | One publisher thread and one observer thread. |
