#!/usr/bin/env python3
"""Read-only backup, restore, and PITR drill readiness checker."""
...
LIMITATIONS = (
    "This script is read-only and does not restore data, replay WAL, mutate artifacts, or open application traffic.",
    "Passing this script is local readiness evidence only; it is not release proof for production backup/PITR readiness.",
    "Release proof still requires a recorded restore drill with exact backup id, snapshot id, WAL range, target LSN, RestoreTrace, RecoveryReport, and opening decision.",
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Read-only Andromeda backup/restore/PITR drill checker. "
...
def build_report(root: Path) -> dict[str, object]:
        ...
        if not result.exists:
            gaps.append(
                Gap(
                    "path",
                    result.path,
                    "Required backup/restore/PITR drill artifact is missing.",
                )
            )
    ...
    return {
        "schema": "andromeda.backup_restore_drill_check.v1",
        "root": str(root),
        "mode": "read-only",
...
def print_text_report(report: dict[str, object]) -> None:
    print("Andromeda Backup/Restore Drill Check")
    ...
    for limitation in report["limitations"]:
        print(f"  {limitation}")
    print()
    print("Result: backup/restore drill check completed.")
