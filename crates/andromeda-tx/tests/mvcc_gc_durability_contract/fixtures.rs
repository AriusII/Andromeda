...
use andromeda_tx::{
    ActiveSnapshotRegistry, Lsn, MvccGarbageCollector, SnapshotHandle, TransactionStatusTable,
};
use std::sync::Arc;
...
pub(crate) fn register_snapshot(
    ...
    let snapshot = SnapshotHandle::new(timestamp, tx_id).expect("create snapshot");
    registry
