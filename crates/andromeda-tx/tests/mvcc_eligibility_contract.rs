...
use andromeda_tx::mvcc_eligibility::{
    VersionEligibilityChecker, VersionEligibility, VersionRecord,
};
use andromeda_tx::{ActiveSnapshotRegistry, TransactionStatusTable, TransactionStatus, SnapshotHandle};
...
fn test_creator_rolled_back_false() {
    ...
    let tx_id = TransactionIdGenerator::new().generate();
    status_table.record(tx_id, TransactionStatus::RolledBack).unwrap();
...
fn test_end_ts_equal_to_min_visible_not_invisible() {
    ...
    let snapshot_tx_id = TransactionIdGenerator::new().generate();
    let snapshot_handle = SnapshotHandle::new(100, snapshot_tx_id).unwrap();
    snapshot_registry.register_snapshot(snapshot_handle).unwrap();
...
fn test_fully_eligible_with_multiple_snapshots() {
    ...
    let snapshot_handles: Vec<_> = (0..5)
        .map(|i| {
            let stx_id = TransactionIdGenerator::new().generate();
...
fn test_stats_accumulate_correctly() {
    ...
    let tx_id_committed = TransactionIdGenerator::new().generate();
    status_table.set_committed(tx_id_committed).unwrap();

    let tx_id_inflight = TransactionIdGenerator::new().generate();
...
fn test_aborted_version_immediately_eligible() {
    ...
    let snapshot_registry = make_snapshot_registry();
    let checker =
        VersionEligibilityChecker::new(status_table.clone(), snapshot_registry, 100).unwrap();
...
fn test_concurrent_aborted_versions_stress() {
    ...
    let checker = Arc::new(
        VersionEligibilityChecker::new(status_table.clone(), snapshot_registry.clone(), 2)
            .unwrap(),
    );
    ...
    let handles: Vec<_> = (0..50)
        .map(|i| {
            ...
            thread::spawn(move || {
                ...
                if i % 2 == 0 {
                    status_table_clone.record(tx_id, TransactionStatus::RolledBack).unwrap();
                }
                
                let version = VersionRecord::new(i as u64, tx_id, 100 + i as u64, 5).unwrap();
...
fn test_invariant_no_in_flight_version_reclaimed() {
    ...
    let snapshot_registry = make_snapshot_registry();
    let checker =
        VersionEligibilityChecker::new(status_table.clone(), snapshot_registry, 0).unwrap();
...
fn test_first_failed_criterion_aborted_supersedes_other_failures() {
    ...
    let snapshot_registry = make_snapshot_registry();
    let checker =
        VersionEligibilityChecker::new(status_table.clone(), snapshot_registry, 1000).unwrap();
...
fn test_stats_track_aborted_and_in_flight_checks() {
    ...
    let tx_aborted = TransactionIdGenerator::new().generate();
    status_table.record(tx_aborted, TransactionStatus::RolledBack).unwrap();

    let tx_inflight = TransactionIdGenerator::new().generate();
    
    let tx_committed = TransactionIdGenerator::new().generate();
