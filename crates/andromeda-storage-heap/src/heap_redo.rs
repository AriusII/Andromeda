...
pub struct HeapRowRedoPayloadV1 {
    pub operation: HeapRowRedoOperation,
    pub page_id: PageId,
    pub page_size: PageSize,
    pub before_slot_id: u16,
    pub after_slot_id: u16,
    pub expected_previous_page_lsn: Lsn,
    pub resulting_page_lsn: Lsn,
    pub tuple: Vec<u8>,
}
...
mod tests {
    ...
    #[test]
    fn row_insert_round_trips_with_wal_kind() {
        ...
        let encoded = payload
            ...
            .expect("encode");
        let decoded =
            HeapRowRedoPayloadV1::decode(&encoded, WalRecordKind::RowInsert).expect("decode");

        assert_eq!(decoded.operation(), HeapRowRedoOperation::Insert);
        assert_eq!(decoded.page_id(), PageId::new(42));
        assert_eq!(decoded.page_size(), PageSize::KiB16);
        assert_eq!(decoded.after_slot_id(), 7);
        assert_eq!(decoded.resulting_page_lsn(), Lsn::new(10));
        assert_eq!(decoded.tuple(), b"tuple");
    }

    #[test]
    fn row_delete_requires_empty_tuple() {
        let mut encoded = HeapRowRedoPayloadV1::row_delete(
            PageId::new(42),
            PageSize::KiB16,
            7,
            Lsn::new(9),
        ...
        let err = HeapRowRedoPayloadV1::decode(&encoded, WalRecordKind::RowDelete)
            .expect_err("tuple length mismatch");
        assert!(err.message().contains("payload length"));
    }

    #[test]
    fn mismatched_wal_kind_is_rejected() {
        let payload = HeapRowRedoPayloadV1::row_insert(
            PageId::new(42),
            PageSize::KiB16,
            7,
        ...
        assert!(err.message().contains("cannot be encoded"));
    }
}
