...
- `andromeda-protocol` is a runtime-free scaffold for typed protocol-level definitions shared across transport and procedure layers. It must not own QUIC runtime behavior, generated Protobuf code, persistent state, WAL authority, recovery behavior, or catalog publication.
...
- `andromeda-rpc-codec` owns runtime-free RPC payload codec helpers and typed envelope validation below orchestration. It must not own transport sockets, retries, admission, Procedure execution, catalog mutation, WAL, storage, or recovery behavior.
- `andromeda-rpc` owns runtime-free RPC dispatch glue between frame protocol validation and typed payload envelope validation. It must not own byte-level codecs, QUIC/TLS runtime behavior, Procedure execution, catalog writes, WAL durability, or recovery behavior.
...
- `andromeda-admin` reserves the administration facade boundary. It currently owns marker-only administration crate identity and must not expose application runtime behavior, HA/DR behavior, storage access, or Procedure execution.
- `andromeda-client-sdk-gen` reserves the runtime-free client SDK generation boundary. It must not own transport runtime behavior, server execution, catalog state, WAL, recovery, or unversioned generated artifacts.
...
- `andromeda-wal-codec` owns the raw WAL binary frame codec, little-endian field encoding, header validation, and bounded frame scan loop. It must not own WAL record semantics, fsync behavior, append orchestration, replay policy, checkpointing, storage recovery, or catalog durability policy.
...
- `andromeda-audit` owns typed audit event vocabulary for review and forensic correlation. It must not own durable audit journal storage, retention, compaction, exporters, authorization truth, storage truth, WAL truth, or recovery truth.
- `andromeda-observability` owns shared observability identifiers and correlation metadata. It must not own instrumentation runtimes, exporters, metrics collection, durable audit journals, query behavior, or authoritative subsystem state.
...
- `andromeda-iam` owns the minimal pre-transaction IAM admission evaluator over runtime-free security contract vocabulary. It must not own certificate parsing, mTLS extraction, mutable registry persistence, audit sinks, transactions, WAL, storage, or recovery.
- `andromeda-security` is a scaffold boundary for future security domain work. It currently must remain dependency-free and must not duplicate `andromeda-security-contract` or `andromeda-iam` ownership.
- `andromeda-gpu` owns optional GPU advisory selection boundaries for analytical work. It must keep CPU fallback and cancellation requirements explicit and must not enter commit, WAL, rollback, recovery, MVCC visibility, catalog publication, or security-critical paths.
- `andromeda-simd` owns optional SIMD dispatch boundaries for analytical CPU acceleration. Scalar execution remains the required fallback and SIMD output must not become durable, catalog, transaction, recovery, or security truth.
- `andromeda-vector` owns advisory vector-output validation. Vector evidence may rank, prune, or suggest analytical candidates, but must not become durable, transaction, catalog, recovery, or security truth.
...
- `andromeda-business-fixtures` owns reusable business-domain fixtures for tests and examples. It must not become production execution logic, storage truth, or an alternate business runtime.
- `andromeda-test-support` owns mechanical, runtime-free test support helpers. It must not own production logic, doctrine constants, command semantics, benchmark authority, or durable runtime truth.
