...
fn validate_page_flush_boundary(page_lsn: Lsn, durable_lsn: Lsn) -> AndromedaResult<()> {
    PageFlushDurabilityBoundary::new(page_lsn, durable_lsn)
        ...
        .map_err(|error| match error {
            crate::PageFlushDurabilityError::MissingPageLsn => {
                DiskManagerError::PageLayoutInvalid {
                    reason: "page flush requires a nonzero page LSN".to_string(),
                }
                .into()
            },
            crate::PageFlushDurabilityError::WalFenceViolation {
                page_lsn,
                durable_lsn,
...
mod tests {
    ...
    use andromeda_storage_page::{
        AllocationId, ObjectId, PageFlags, PageHeader, PageLayoutContract, PageSize, PageTrailer,
        PageType,
    };

    use crate::{DiskManager, DiskManagerError};
    ...
    fn valid_page(page_id: PageId, page_lsn: Lsn) -> PageImage {
        ...
        PageImage::with_layout(layout, vec![0; PageSize::KiB16.bytes_usize()])
