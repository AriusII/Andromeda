...
//!
//! This module implements all encode and decode logic for the durable catalog
//! mutation payload format: header framing, FNV-1a checksum, and per-variant
//! body serialization.  All integers are little-endian.  The `Decoder` cursor
//! type enforces exhaustion of every body byte so truncation or trailing data
