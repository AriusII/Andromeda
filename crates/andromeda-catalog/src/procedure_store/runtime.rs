...
/// Stable identifier for one terminal runtime record.
///
/// The identifier is derived from the invocation id, full procedure binding,
/// optional decision trace id, optional plan key/id pair, counters, timing, and
/// terminal outcome. It is deterministic evidence only; it is not a decision
/// authority and it is not a WAL durability proof.
...
///
/// The catalog does not own runtime plan storage.  This type only records the
/// bounded, privacy-safe identifier that an optimizer or executor can derive
/// from a [`PlanCacheKey`].
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
/// Runtime counters captured from one completed invocation.
///
/// All counters are observed evidence.  They are not authoritative optimizer
/// decisions, and they do not alter catalog truth by themselves.
...
impl ProcedureRuntimeCounters {
    ...
    pub const fn evidence_role(self) -> ProcedureStoreEvidenceRole {
        ProcedureStoreEvidenceRole::observed_feedback()
    }
...
/// Closed terminal status taxonomy for Procedure Store runtime records.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
/// [`ProcedureContractBinding`] tuple.  Consumers can therefore reject stale
/// records when any contract, catalog, statistics, or policy identity drifts.
