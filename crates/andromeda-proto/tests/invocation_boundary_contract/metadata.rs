...
use andromeda_proto_wire::validate_generated_invocation_request;
...
fn generated_invocation_request_requires_binding_identity_match() {
    ...
    validate_generated_invocation_request(&valid).unwrap();

    let mut wrong_hash = valid.clone();
    wrong_hash.correlation.as_mut().unwrap().contract_hash = Some(hash(0x22));
    let wrong_hash_error = validate_generated_invocation_request(&wrong_hash)
    ...
    let wrong_catalog_error = validate_generated_invocation_request(&wrong_catalog)
    ...
    let wrong_stats_error = validate_generated_invocation_request(&wrong_stats)
    ...
    assert!(validate_generated_invocation_request(&missing_stats).is_err());
}
...
fn generated_invocation_request_requires_expected_policy_version() {
    ...
    assert!(validate_generated_invocation_request(&missing_policy).is_err());
    ...
    assert!(validate_generated_invocation_request(&zero_policy).is_err());
}
