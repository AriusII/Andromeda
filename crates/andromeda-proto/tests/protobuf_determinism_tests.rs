...
use andromeda_proto::generated::andromeda::protocol::v1::{
    FrameEnvelope, ProtocolVersion, PayloadKind, RpcExecuteRequest, RpcBatch,
    RpcCompletion, ErrorEnvelope,
};

use andromeda_proto::generated::andromeda::contract::v1::{
    ProcedureManifest, StructuredObjectHeader, ResultStreamDescriptor,
};
...
fn test_frame_envelope_edge_cases() {
    ...
    let decoded = FrameEnvelope::decode(bytes.as_slice())
        .expect("Deserialize with empty hash");
...
fn test_rpc_completion_all_fields() {
    ...
    let decoded = RpcCompletion::decode(bytes.as_slice())
...
fn test_structured_object_header_deterministic_serialization() {
    ...
    assert_eq!(bytes1, bytes2, "Structured object headers must serialize identically");
}
...
fn test_known_message_encoding_contracts() {
    ...
    // ProtocolVersion(1, 0) must always encode to 0x0A 0x02 0x08 0x01
    let version_1_0 = ProtocolVersion { major: 1, minor: 0 };
    let expected_bytes = vec![0x0A, 0x02, 0x08, 0x01];
...
fn test_field_presence_preservation() {
    ...
    assert_ne!(bytes_with, bytes_without, "Optional field presence must affect encoding");
