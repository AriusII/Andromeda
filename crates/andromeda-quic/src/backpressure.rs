...
//! control streams — and when carried over a QUIC DATAGRAM it must fit
...
/// request id and survive loss) or fired and forgotten on a telemetry
/// datagram (where the encoded form must fit the negotiated MTU).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BackpressureTransport {
    /// Reliable diagnostic QUIC stream.
    DiagnosticStream,
    /// QUIC DATAGRAM with a negotiated maximum payload size in bytes.
...
impl BackpressureSignal {
    ...
    /// Minimum QUIC DATAGRAM payload that the project will negotiate before
    /// allowing backpressure to be carried over a datagram. This is well
    /// below the IETF QUIC `max_datagram_frame_size` floor used by Andromeda
    /// (1200 bytes), but we expose it as a named constant so the routing
    /// invariant is testable without binding to a concrete I/O runtime.
    ...
    /// bounds. On a QUIC DATAGRAM it additionally rejects negotiated MTUs
    /// that cannot accommodate the encoded form, and rejects any signal
    /// whose reason is request-scoped without a RequestId (because a
    /// fire-and-forget datagram cannot fall back to a reliable retry).
