...
`andromeda-quic` owns Andromeda's runtime-free QUIC transport boundary, including transport contracts, stream role validation, lifecycle gating, backpressure, Procedure gateway admission, reconnect policy, and HA/DR stream allocation.
...
| Runtime split | Compatibility reexports for RPC contracts; concrete Quinn, Rustls, Tokio, socket, and TLS behavior lives in `andromeda-quic-runtime-quinn`. |
...
| Typed envelopes | Reexports typed envelope validation owned by `andromeda-rpc-codec`. |
...
`andromeda-quic` reexports selected `andromeda-rpc-protocol`, `andromeda-rpc-codec`, and `andromeda-rpc` contracts as compatibility facades during migration. The canonical runtime-free frame owner remains `andromeda-rpc-protocol`; typed envelope codec glue lives in `andromeda-rpc-codec`; dispatch glue lives in `andromeda-rpc`.
...
- Do not add Quinn, Rustls, Tokio, or socket runtime ownership to this crate.
...
1. Keep runtime-free contracts available without concrete runtime dependencies.
2. Put concrete Quinn, Rustls, Tokio, socket, and TLS behavior in `andromeda-quic-runtime-quinn`.
...
For Quinn-backed behavior, also run the runtime crate tests:
...
| Quinn types leak into `andromeda-quic`. | Move them into `andromeda-quic-runtime-quinn` and keep runtime-free tests passing without concrete runtime dependencies. |
