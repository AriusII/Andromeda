...
use crate::{CatalogReplayFromLsnReport, CatalogStorageWalRecord, LsnBoundCatalogRecord};

use super::CatalogSnapshot;
use super::state::CatalogReplayState;

pub type CatalogStorageReplayLsnRecord = LsnBoundCatalogRecord<CatalogStorageWalRecord>;
pub type CatalogStorageReplayFromLsnReport = CatalogReplayFromLsnReport<CatalogSnapshot>;
...
pub fn replay_catalog_wal_records(
    records: &[CatalogStorageWalRecord],
...
/// 2. Extract the `CatalogStorageWalRecord` slice from the eligible entries (order preserved).
/// 3. Call [`replay_catalog_wal_records`] on the filtered slice.
/// 4. Return [`CatalogStorageReplayFromLsnReport`] with replay statistics.
///
/// # Preconditions
///
...
pub fn replay_catalog_from_lsn(
    lsn_records: &[CatalogStorageReplayLsnRecord],
    catalog_wal_start_lsn: Lsn,
    target_catalog_version: CatalogVersion,
) -> AndromedaResult<CatalogStorageReplayFromLsnReport> {
    let selection = crate::CatalogLsnReplaySelection::select(lsn_records, catalog_wal_start_lsn)?;
    let snapshot =
        replay_catalog_wal_records(selection.eligible_records(), target_catalog_version)?;

    Ok(CatalogStorageReplayFromLsnReport {
