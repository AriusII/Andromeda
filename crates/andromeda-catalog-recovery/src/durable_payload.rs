//! Explicit durable codec for catalog mutation WAL payloads.
...
use andromeda_procedure_contract::{
    ...
    ProcedureErrorPolicy, ProtocolLayoutRef, ResultMetadataPolicy, ResultStreamCardinality,
...
use crate::{
    ...
    mutation::{validate_boundary, validate_delta, validate_record},
};

const CATALOG_WAL_PAYLOAD_MAGIC: u64 = 0x414e_4452_4341_5457; // "ANDRCATW"
const CATALOG_WAL_PAYLOAD_VERSION_V1: u16 = 1;
const CATALOG_WAL_PAYLOAD_VERSION_V2: u16 = 2;
const CATALOG_WAL_PAYLOAD_VERSION_V3: u16 = 3;
const CATALOG_WAL_PAYLOAD_VERSION_V4: u16 = 4;
const CATALOG_WAL_PAYLOAD_VERSION_CURRENT: u16 = CATALOG_WAL_PAYLOAD_VERSION_V4;
const CATALOG_WAL_PAYLOAD_HEADER_LEN: usize = 28;

pub fn encode_catalog_durable_payload(record: &CatalogMutationRecord) -> AndromedaResult<Vec<u8>> {
    validate_catalog_mutation_record(record)?;
...
pub fn decode_catalog_durable_payload(
    payload: &[u8],
...
pub fn validate_catalog_mutation_record(record: &CatalogMutationRecord) -> AndromedaResult<()> {
...
struct PayloadHeader {
    version: u16,
    kind: CatalogMutationRecordKind,
    kind_tag: u16,
    body_len: u64,
    checksum: u64,
}

fn encode_payload_header(out: &mut Vec<u8>, kind_tag: u16, body_len: u64, checksum: u64) {
...
fn decode_payload_header(payload: &[u8]) -> Result<PayloadHeader, CatalogWalPayloadDecodeError> {
...
fn decode_boundary(decoder: &mut Decoder<'_>) -> AndromedaResult<CatalogMutationBoundary> {
    let boundary = CatalogMutationBoundary {
        batch_id: andromeda_definition_batch::DefinitionBatchId::new(decoder.u64()?),
        ...
        source_hash: andromeda_definition_batch::DefinitionBatchSourceHash::new(
            decode_sha256_array(decoder)?,
        ),
...
fn catalog_wal_payload_checksum(version: u16, kind_tag: u16, body_len: u64, body: &[u8]) -> u64 {
...
fn catalog_error(message: impl Into<String>) -> AndromedaError {
