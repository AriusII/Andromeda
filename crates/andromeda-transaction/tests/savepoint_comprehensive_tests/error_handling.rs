...
fn eh_failed_tx_rollback_completion_requires_durable_wal() {
    ...
    let err = machine
        .apply(andromeda_transaction::TransactionEvent::RollbackComplete)
        .unwrap_err();
...
fn eh_fail_on_created_state_is_rejected() {
    ...
    let state = andromeda_transaction::TransactionState::Created;
    let err = state
        .apply(andromeda_transaction::TransactionEvent::Fail)
        .unwrap_err();
    assert_eq!(err.kind(), AndromedaErrorKind::Transaction);
}
