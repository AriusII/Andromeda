...
use super::{
    AdvisoryEvidenceSummary, PLAN_SELECTION_MAX_CANDIDATES, PlanCacheKey, PlanClass,
    PlanDecisionEvidence, PlanDecisionOutcome,
};
...
/// using a pre-validated advisory evidence summary for trace output.
///
/// Advisory evidence never overrides the bounded static rank. Ties are broken
/// by `PlanCandidateId` for deterministic cross-node behavior.
pub fn select_minimal_plan_with_advisory_evidence(
    key: PlanCacheKey,
