use andromeda_proto_wire::{
    GeneratedFrameEnvelope, GeneratedPayloadKind, GeneratedProtocolVersion, PayloadKind,
    ProtocolVersion, decode_protobuf_message, encode_protobuf_message,
...
fn validate_envelope_safely(data: &[u8]) -> EnvelopeDecision {
    let proto: GeneratedFrameEnvelope = match decode_protobuf_message(data, "frame envelope") {
        Ok(proto) => proto,
...
fn validate_proto_envelope(proto: GeneratedFrameEnvelope) -> EnvelopeDecision {
...
fn valid_proto_envelope(
    payload_kind: GeneratedPayloadKind,
    payload: impl Into<Vec<u8>>,
) -> GeneratedFrameEnvelope {
    GeneratedFrameEnvelope {
...
fn encoded_valid_rpc_execute(payload: Vec<u8>) -> Vec<u8> {
    encode_protobuf_message(&valid_proto_envelope(
...
fn prop_valid_generated_envelopes_round_trip_before_validation() {
    proptest!(|(payload in arb_payload())| {
        let proto = valid_proto_envelope(GeneratedPayloadKind::RpcBatch, payload);
        let bytes = encode_protobuf_message(&proto);
        let decoded: GeneratedFrameEnvelope =
            decode_protobuf_message(&bytes, "frame envelope").expect("valid generated envelope decodes");
...
fn prop_unsupported_protocol_versions_are_rejected() {
    proptest!(|(
        ...
        let mut proto = valid_proto_envelope(GeneratedPayloadKind::RpcExecuteRequest, payload);
        proto.protocol_version = Some(GeneratedProtocolVersion { major, minor });
...
fn contract_bound_payloads_reject_zero_hash() {
    let mut proto = valid_proto_envelope(GeneratedPayloadKind::RpcCompletion, Vec::new());
...
fn contract_bound_payloads_reject_zero_runtime_binding_ids() {
    let cases = [
        (
            ...
            GeneratedFrameEnvelope {
                catalog_version: 0,
                ..valid_proto_envelope(GeneratedPayloadKind::RpcCompletion, Vec::new())
            },
        ),
        (
            "request_id",
...
fn generated_frame_validator_rejects_missing_protocol_version() {
    let proto = GeneratedFrameEnvelope {
...
fn rpc_batch_requires_non_empty_payload() {
    let proto = valid_proto_envelope(GeneratedPayloadKind::RpcBatch, Vec::new());
    let decision = validate_proto_envelope(proto);
...
fn rpc_completion_accepts_empty_payload_with_contract_binding() {
    let proto = valid_proto_envelope(GeneratedPayloadKind::RpcCompletion, Vec::new());
    let decision = validate_proto_envelope(proto);
