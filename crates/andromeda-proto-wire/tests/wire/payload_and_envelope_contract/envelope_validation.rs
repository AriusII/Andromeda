...
use andromeda_proto_wire::{
    FrameEnvelope, GeneratedFrameEnvelope, GeneratedPayloadKind, GeneratedProtocolVersion,
    PayloadKind, validate_generated_frame_envelope,
};
...
const ENVELOPE_SCHEMA: &str =
    include_str!("../../../../andromeda-proto/proto/andromeda/protocol/v1/envelope.proto");
...
fn generated_frame_envelope_rejects_unbound_or_unknown_payload_metadata() {
    let valid = GeneratedFrameEnvelope {
        ...
        session_id: 202,
        tx_id: Some(303),
        payload_kind: GeneratedPayloadKind::RpcBatch as i32,
        payload: b"row-batch".to_vec(),
    };

    validate_generated_frame_envelope(&valid).unwrap();

    let missing_version = GeneratedFrameEnvelope {
    ...
    let zero_catalog_version = GeneratedFrameEnvelope {
        catalog_version: 0,
        ..valid.clone()
    };
    assert_eq!(
    ...
    let unknown_payload_kind = GeneratedFrameEnvelope {
        payload_kind: 200,
        ..valid
    };
    assert_eq!(
