...
use crate::{
    InMemoryProcedureFeedbackStore, InvocationDecisionRecord, InvocationRuntimeRecord,
    InvocationRuntimeRecordOutcome, ProcedureFeedback, ProcedureFeedbackStore,
    ProcedureRegistration, ProcedureStoreEntry, RecordOutcome,
};

/// Default per-procedure capacity for advisory feedback evidence. The store is
/// bounded so observed-outcome feedback cannot grow without limit; chosen as a
/// small power-of-two sample window. Advisory evidence is sampled, never
...
/// neither identity may collide. Re-registering the same exact entry is a no-op
/// (idempotent); any divergence is rejected.
...
impl ProcedureStore {
    ...
    /// Register an entry. Returns [`ProcedureRegistration::Inserted`] for a new
    /// procedure or [`ProcedureRegistration::AlreadyRegistered`] if the exact
    /// same entry was previously registered. Any conflict (id collision with
    /// different metadata, name collision with a different id, or contract hash
    /// divergence for the same id) is rejected.
    pub fn register(
        &mut self,
    ...
    /// Attach decision evidence to an existing procedure. The contract hash and
    /// catalog version on the record must match the registered binding,
    ...
    ///
    /// A runtime record is accepted only when it binds to the exact registered
    /// `ProcedureContractBinding`: `ProcedureId`, `ContractHash`,
    ...
    /// Attach advisory observed-outcome feedback to a registered procedure.
    ///
    /// The feedback must remain non-authoritative, bind to a registered
    /// procedure, and use the same statistics version as the registered
    /// contract binding. Recording is idempotent for identical feedback ids and
    /// digests, conflict-detecting for divergent digests, and bounded per
    /// procedure by [`PROCEDURE_FEEDBACK_CAPACITY_PER_PROCEDURE`].
    ...
    /// Read-only view of advisory feedback recorded for a procedure that is
    /// non-expired at `now`. Returns an empty vector when no feedback is
    /// present.
    ...
    /// Drop expired feedback across every procedure. Returns the total number
    /// of records removed.
