...
use andromeda_storage::{
    Lsn, WalRecord, WalRecordKind, WalScanStopReason, decode_wal_record_frame, encode_wal_record,
    scan_wal_records,
};
...
#[test]
fn canonical_wal_frame_matches_golden_byte_vector() {
    let record = tx_record(2, Some(1), b"bb");
    let expected = vec![
        0x4c, 0x41, 0x57, 0x4f, 0x52, 0x44, 0x4e, 0x41, 0x01, 0x00, 0x48, 0x00, 0x4a, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x03, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x25, 0xb3, 0x48, 0xb4,
        0xb0, 0x65, 0xfc, 0xde, 0x4a, 0xa4, 0x86, 0x44, 0xde, 0xc2, 0x07, 0x16, 0x62, 0x62,
    ];

    assert_eq!(encode_wal_record(&record).unwrap(), expected);

    let (decoded, consumed) = decode_wal_record_frame(&expected).unwrap().unwrap();
