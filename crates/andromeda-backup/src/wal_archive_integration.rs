...
use crate::{
    error::{BackupResult, BackupValidationError, backup_error, map_backup_validation},
    physical_plan::BackupPhysicalPlan,
    plan::BackupManifest as BackupManifestRaw,
    primitives::BackupTraceId,
    types::{BackupId, WalArchiveRange as WalArchiveRangeRaw},
};
...
impl WalArchiveRejection {
    fn into_error(self) -> BackupValidationError {
        backup_error(self.to_string())
    }
}
...
impl WalArchiveIntegration {
    ...
    pub fn validate_wal_archive(
        ...
        segment_count: u64,
    ) -> BackupResult<WalArchiveValidationResult> {
    ...
    pub fn finalize_backup_with_wal_archive<T>(
        plan: &BackupPhysicalPlan<Lsn, T>,
        manifest_template: &BackupManifestRaw<Lsn>,
        shipped_wal_lsn: Lsn,
        wal_segment_count: u64,
    ) -> BackupResult<(BackupManifestRaw<Lsn>, BackupManifestFinalizedEvent)>
    where
        T: BackupTraceId,
    {
        ...
        let finalized_manifest = BackupManifestRaw {
            ...
            wal_archive: WalArchiveRangeRaw::new(
                manifest_template.wal_archive.start,
                shipped_wal_lsn,
            ),
    ...
    pub fn compute_pitr_window(manifest: &BackupManifestRaw<Lsn>) -> BackupResult<(Lsn, Lsn)> {
    ...
    pub fn validate_pitr_target(
        manifest: &BackupManifestRaw<Lsn>,
        target_lsn: Lsn,
    ) -> BackupResult<()> {
        map_backup_validation(manifest.validate())?;
...
mod tests {
    ...
    use crate::{
        BackupManifest,
        types::{ColdSnapshotBoundary, WalArchiveRange},
    };

    #[test]
    fn test_wal_archive_validation() -> BackupResult<()> {
    ...
    #[test]
    fn test_pitr_window_computation() -> BackupResult<()> {
