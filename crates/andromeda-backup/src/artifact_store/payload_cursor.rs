...
pub(super) fn read_u16_at(bytes: &[u8], offset: usize) -> BackupResult<u16> {
    let end = offset
        .checked_add(2)
...
pub(super) fn read_u64_at(bytes: &[u8], offset: usize) -> BackupResult<u64> {
    let end = offset
        .checked_add(8)
        .ok_or_else(|| backup_error("backup artifact manifest offset overflow"))?;
...
impl<'a> PayloadCursor<'a> {
    ...
    pub(super) fn finish(&self) -> BackupResult<()> {
    ...
    fn read_exact(&mut self, len: usize) -> BackupResult<&'a [u8]> {
        let end = self
            .offset
            .checked_add(len)
    ...
    pub(super) fn read_u64(&mut self) -> BackupResult<u64> {
        let array: [u8; 8] = self
            .read_exact(8)?
            .try_into()
            .map_err(|_| backup_error("backup artifact manifest u64 field is truncated"))?;
    ...
    pub(super) fn read_u32(&mut self) -> BackupResult<u32> {
        let array: [u8; 4] = self
            .read_exact(4)?
            .try_into()
    ...
    pub(super) fn read_u16(&mut self) -> BackupResult<u16> {
        let array: [u8; 2] = self
            .read_exact(2)?
            .try_into()
            .map_err(|_| backup_error("backup artifact manifest u16 field is truncated"))?;
    ...
    fn read_u8(&mut self) -> BackupResult<u8> {
        Ok(*self
            .read_exact(1)?
            .first()
    ...
    pub(super) fn read_lsn(&mut self) -> BackupResult<Lsn> {
        Ok(Lsn::new(self.read_u64()?))
    }

    pub(super) fn read_option_lsn(&mut self) -> BackupResult<Option<Lsn>> {
    ...
    pub(super) fn read_array_32(&mut self) -> BackupResult<[u8; 32]> {
        self.read_exact(32)?
            .try_into()
            .map_err(|_| backup_error("backup artifact manifest 32-byte field is truncated"))
    }

    pub(super) fn read_digest(&mut self) -> BackupResult<BackupArtifactDigest> {
