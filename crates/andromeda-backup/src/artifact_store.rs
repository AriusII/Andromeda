...
pub type BackupPhysicalArtifactSet = BackupPhysicalArtifactSetRaw<Lsn>;
pub type BackupWalSegmentArtifact = BackupWalSegmentArtifactRaw<Lsn>;
use manifest_format::{
...
impl BackupArtifactManifestRecord {
    ...
    pub fn wal_segment_descriptors(&self) -> BackupResult<Vec<WalSegmentDescriptor>> {
...
impl FileBackedBackupArtifactStore {
    pub fn open(root: impl Into<PathBuf>) -> BackupResult<Self> {
    ...
    pub fn open_existing(root: impl Into<PathBuf>) -> BackupResult<Self> {
        let root = root.into();
        if !root.exists() {
    ...
    pub fn write_execution_plan_artifact<S>(
        &self,
    ...
    pub fn load_artifact_manifest(
        &self,
        backup_id: BackupId,
    ) -> BackupResult<BackupArtifactManifestRecord> {
    ...
    fn validate_artifact_files(&self, record: &BackupArtifactManifestRecord) -> BackupResult<()> {
...
fn build_wal_archive_evidence(
    manifest: &BackupManifest,
    wal_segments: &[BackupWalSegmentArtifact],
) -> BackupResult<BackupWalArchiveEvidence> {
...
fn total_wal_artifact_bytes(wal_segments: &[BackupWalSegmentArtifact]) -> BackupResult<u64> {
