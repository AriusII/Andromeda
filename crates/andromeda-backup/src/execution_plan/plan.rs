...
use super::{
    ...
    tasks::{BackupSourceTier, BackupStorageTier, ExtentCopyTask, WalSegmentCopyTask},
};
use crate::{
    error::{BackupResult, backup_error, map_backup_validation, map_core_validation},
...
pub struct BackupExecutionPlan<S = BackupStorageTier> {
    /// The backup manifest (snapshot + WAL range boundaries)
    pub manifest: BackupManifestRaw<Lsn>,

    /// Extent copy tasks for the cold snapshot (can execute in parallel)
    pub extent_copy_plan: Vec<ExtentCopyTask<S>>,
...
impl<S: BackupSourceTier> BackupExecutionPlan<S> {
    ...
    fn validate_extent_copy_plan(&self) -> BackupResult<u64> {
    ...
    fn validate_wal_copy_plan(&self) -> BackupResult<u64> {
    ...
    fn validate_wal_segment_chain(&self) -> BackupResult<()> {
        ...
        for (index, task) in self.wal_segment_copy_plan.iter().enumerate() {
            ...
            map_core_validation(desc.validate())?;
            ...
            expected_first = map_core_validation(desc.last_lsn.try_next())?;
        }
    ...
    pub fn total_bytes(&self) -> BackupResult<u64> {
