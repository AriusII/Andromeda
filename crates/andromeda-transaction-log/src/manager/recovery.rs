//! Commit-log recovery and status-table reconstruction.
//!
...
use crate::{
    CommitLogEntry, Lsn, RollbackLogEntry, TransactionLogStatus, TransactionStatusRebuild,
    TransactionStatusStore, TxWalReplayAction, TxWalReplayRecord, TxWalReplaySummary,
};
...
impl<S: TransactionStatusStore> CommitLogManager<S> {
    ...
    pub fn rebuild_status_from_records(&self) -> AndromedaResult<TransactionStatusRebuild> {
        ...
        for entry in self.commit_entries.iter() {
            ...
            match self.status_table.status(tx_id) {
                Some(TransactionLogStatus::Committed) => summary.already_present += 1,
        ...
        for entry in self.rollback_entries.iter() {
            ...
            match self.status_table.status(tx_id) {
                Some(TransactionLogStatus::RolledBack) => summary.already_present += 1,
                Some(_) => return Err(conflicting_terminal_record()),
                None => {
    ...
    fn replay_commit_record(&self, entry: CommitLogEntry) -> AndromedaResult<TxWalReplayAction> {
        ...
        validate_terminal_status_compatible(
            &self.status_table,
            entry.tx_id,
            TransactionLogStatus::Committed,
        )?;
        ...
        self.status_table
            .restore_terminal_from_validated_replay(entry.tx_id, TransactionLogStatus::Committed)?;
        Ok(action)
    }
    ...
    fn replay_rollback_record(
        ...
        validate_terminal_status_compatible(
            &self.status_table,
            entry.tx_id,
            TransactionLogStatus::RolledBack,
        )?;
        ...
        self.status_table.restore_terminal_from_validated_replay(
            entry.tx_id,
            TransactionLogStatus::RolledBack,
        )?;
        Ok(action)
