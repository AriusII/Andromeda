...
/// Error types specific to transaction WAL adapter and replay failures.
///
/// New adapter implementations can use these to categorize their errors before
/// converting to `AndromedaError`.
...
pub(crate) fn tx_adapter_error(error: TxWalAdapterError) -> AndromedaError {
    error.into_andromeda_error()
}
