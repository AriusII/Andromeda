use andromeda_core::{PipelineClass, RamSectionRole, ResourceBudget};
    ...
    pub fn validate_admission(
        ...
        if self.pipeline_class != workload_pipeline {
            return Err(InvocationReject {
                status: crate::CompletionStatus::ContractRejected,
                reason: format!(
...
fn resource_reject_from_error(error: andromeda_core::AndromedaError) -> InvocationReject {
    resource_reject(error.to_string())
}

fn resource_reject(reason: impl Into<String>) -> InvocationReject {
    InvocationReject {
        status: crate::CompletionStatus::SystemUnavailable,
        reason: reason.into(),
    }
}
...
mod tests {
    use super::*;
    use andromeda_core::{GpuExecutionPolicy, GpuProfile, RequestId, SessionId};
