...
fn crash_manifest_truncate_partial_write() {
    ...
    assert_eq!(
        recovered_manifest.checkpoint_lsn(),
        old_manifest.checkpoint_lsn()
    );
    assert_eq!(
        recovered_manifest.recovery_floor_lsn(),
        old_manifest.recovery_floor_lsn()
    );
...
fn crash_manifest_checksum_corrupted() {
    ...
    assert_ne!(corrupted.manifest_crc, manifest.manifest_crc);
...
fn crash_recovery_checkpoint_creation_interrupted() {
    ...
    assert!(new_checkpoint_attempt > old_checkpoint.recovery_floor_lsn().get());
