...
use andromeda_recovery::{
    ConceptualRedoPlan, RecoveryPlan, RecoveryWalReplayAdapter, ReplayContext, ReplayOutcome,
...
fn execute_redo_plan_into_context(
    plan: &ConceptualRedoPlan,
    durable_records: &[WalRecord],
    ctx: &mut ReplayContext,
) -> AndromedaResult<WalReplayReport> {
    let mut adapter = DeferredPageReplayAdapter;
    execute_redo_plan_with_adapter(plan, durable_records, ctx, &mut adapter)
}

struct DeferredPageReplayAdapter;

impl RecoveryWalReplayAdapter<ReplayContext> for DeferredPageReplayAdapter {
    fn replay_record(
        &mut self,
        ctx: &mut ReplayContext,
        record: &WalRecord,
