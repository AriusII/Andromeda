...
use super::{ManifestSwitchRecoveryTrace, ReplayContext, ReplayResult, recovery_error};
...
fn parse_manifest_switch_payload(bytes: &[u8]) -> AndromedaResult<ManifestSwitchPayload> {
    ...
    fn read_u64(bytes: &[u8], start: usize) -> AndromedaResult<u64> {
        ...
        let slice = bytes
            .get(start..end)
            .ok_or_else(|| recovery_error("manifest switch payload is truncated"))?;
        let mut array = [0u8; 8];
    ...
    hash.copy_from_slice(
        bytes
            .get(32..64)
            .ok_or_else(|| recovery_error("manifest switch payload hash is truncated"))?,
    );
