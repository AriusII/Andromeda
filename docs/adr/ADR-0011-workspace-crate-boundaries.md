...
The first implementation batch introduces foundation crates, keeps `andromeda-core` as a compatibility facade, and adds dependency-topology validation. Lot 2.1 introduces `andromeda-contract` as the R1a contract model crate and keeps `andromeda-catalog` reexports for compatibility. Lot 2.2 introduces `andromeda-structured-object` as the R1b owner for contract-safe StructuredObject metadata, descriptor hashing, and row-count metadata policy while `andromeda-proto` keeps schema generation and compatibility reexports. Lot 2.3 classifies `andromeda-security-contract` as the R1 runtime-free security contract vocabulary crate, with the current `andromeda-core` principal facade dependency documented as a temporary exception. Lot 3 introduces SRPL language-model crates for diagnostics, cardinality, AST, parser, and IR while `andromeda-srpl` remains the compatibility facade for binder, lowering, optimizer, interpreter, DefinitionBatch bridge, and historical public imports. Lot 4.0 adds C5 durable-kernel dependency governance before C5 extraction. Lot 4.3 introduces `andromeda-wal` as the owner for pure WAL primitives, LSNs, records, segment descriptors, frame codecs, scan-prefix validation, record bounds, durability fences, and in-memory WAL summaries while `andromeda-storage` keeps compatibility reexports. Lot 4.4 refreshes evidence ownership after that extraction: `andromeda-wal` owns pure WAL owner evidence, while `andromeda-storage` owns recovery reports, replay planning, manifest/page integration, and durable visibility integration evidence. Lot 4.5 moves the physical FileWal byte contract and file-backed WAL append/open surface to `andromeda-wal`, while recovery reports, startup planning, replay selection, manifest/page integration, and durable visibility integration remain owned by `andromeda-storage`. These batches do not change runtime behavior, persistent formats, network formats, FileWal disk semantics, recovery semantics, storage semantics, Procedure dispatch, or public compatibility paths.
...
- Do not move recovery reports, startup planning, replay selection, manifest/page integration, durable visibility integration, catalog store, RPC, execution, SRPL, benchmark, or analytics runtime code in these batches.
- Do not change the FileWal disk format, WAL record format, recovery behavior, page format, manifest format, or visible commit semantics as part of Lot 4.5.
- Do not start Lot 5 recovery, page-layout, buffer-pool, cold-store, MVCC, or storage runtime extraction.
...
| R1 Contracts and language model | Procedure contracts, runtime-free security vocabulary, catalog object descriptors, SRPL AST/IR contracts, protocol schemas, abstract RPC payload contracts | `andromeda-contract`, `andromeda-security-contract`, `andromeda-structured-object`, `andromeda-catalog`, `andromeda-srpl`, `andromeda-proto` | May depend on R0. Must not depend on execution engines, storage runtime implementations, IAM runtime stores, Quinn runtime, or benchmark crates. |
...
`andromeda-security-contract` is the R1 runtime-free owner for security vocabulary: surfaces, permission families, operation classes, canonical permission identifiers, stable labels, and explicit semantic mappings. It is not the IAM runtime and must not own mutable principal registries, role stores, policy stores, revocation stores, audit ledgers, QUIC/TLS runtime behavior, catalog stores, execution, WAL, storage, or recovery behavior. It may remain dependency-free or depend only on contract-safe R0 crates named by the topology guard.
...
Lot 4.0 freezes durable-kernel dependency direction before C5 extraction. Lot 4.3 extracts pure WAL ownership into `andromeda-wal`; `andromeda-storage` remains the compatibility facade for existing WAL import paths and continues to own recovery reports, manifests, page/heap integration, replay planning, and durable visibility integration. Lot 4.4 requires validation and release-gate documents to distinguish owner evidence from integration evidence: storage reexports prove compatibility, not ownership. Lot 4.5 moves physical FileWal ownership into `andromeda-wal` without changing runtime behavior or disk format. The FileWal physical owner evidence is the owner-crate contract for file-backed open/append/header/scan byte invariants. Storage recovery integration evidence remains separate and must prove that storage-owned startup reports, replay plans, manifest/page boundaries, and visibility gates still consume that physical WAL correctly. `andromeda-tx` remains the compatibility facade for transaction callers while future recovery, page-format, buffer-pool, cold-store, and MVCC crates are introduced behind current public imports. A C5 split must first preserve public reexports, then prove byte-for-byte compatibility for persistent formats with roundtrip and golden-vector tests, and finally pass crash/recovery validation for any path that can affect commit visibility, rollback, replay, page flush, manifest publication, or recovery startup.
...
|---|---|---|
| `andromeda-srpl` depends on `andromeda-catalog` for catalog-facing bridge modules. | Lot 3 moves diagnostics, cardinality, AST, parser, and IR into catalog-store-free crates, but binder/lowering, DefinitionBatch dry-run integration, procedure resolver, and contract materialization still live behind the historical facade. | A later SRPL bridge batch moves catalog-facing DefinitionBatch and resolver code behind a dedicated bridge crate, direct callers migrate to the extracted SRPL model crates, and topology guards prove the parser/model/compiler-core crates stay free of catalog store dependencies and unapproved dev-dependencies. |
| `andromeda-quic` reexports `andromeda-rpc-protocol` frame and stream types. | The transport crate is still a compatibility facade for historical imports while `andromeda-rpc-protocol` becomes the runtime-free owner of frame contracts, stream roles, codecs, and ResultStream sequencing. | Callers import frame and stream contracts directly from `andromeda-rpc-protocol`; API compatibility tests prove no public import regressions; `andromeda-quic` keeps only concrete transport adapter behavior and no Procedure, storage truth, WAL, or authorization ownership. |
...
| `andromeda-core` still owns the `principal` module and depends on `andromeda-security-contract`. | Principal identity types were not part of the batch 1 foundation extraction and remain behind the compatibility facade for existing callers. The dependency is accepted only because `andromeda-security-contract` is runtime-free R1 vocabulary, not IAM runtime state. | A later security/IAM batch extracts principal identity and policy contracts to a dedicated crate, migrates callers off the core facade, removes the direct `andromeda-core` to `andromeda-security-contract` exception, then `andromeda-core` becomes reexports only or is retired. |
...
| `andromeda-storage` depends on `andromeda-wal`. | Lot 4.3 makes `andromeda-wal` the canonical owner for pure WAL primitives and codecs while storage keeps compatibility reexports. Lot 4.4 makes this an evidence rule as well: validation documents must attribute pure WAL primitives and codecs to `andromeda-wal`, even when compatibility imports compile through storage. Lot 4.5 extends the owner boundary to the physical FileWal byte contract and file-backed WAL surface, while storage keeps recovery reports, startup/replay planning, manifest/page integration, and durable visibility integration. | A later compatibility cleanup removes storage reexports only after direct callers have migrated to `andromeda-wal` and API compatibility tests prove no public import regressions. |
...
6. Extract durable-kernel code in small phases: API compatibility tests, pure codecs and validators, physical FileWal ownership, recovery planning, replay drivers, transaction payload formats, then I/O/runtime adapters. Lot 4.3 completes the pure WAL owner step only. Lot 4.5 moves the physical FileWal owner surface only; recovery reports, startup planning, replay selection, manifest/page integration, and durable visibility integration remain storage-owned until their dedicated sub-lots.
...
- Crate names must encode ownership and must not introduce generic `common`, `utils`, `misc`, `helpers`, or `god_engine` buckets.
- R0 foundation crates and the temporary `andromeda-core` facade must pass strict dependency allowlists; the only current higher-ring exception is the documented runtime-free `andromeda-security-contract` vocabulary dependency from the principal facade.
...
- `andromeda-security-contract` must stay runtime-free and may not gain IAM runtime, catalog store, execution, WAL, storage, recovery, QUIC/TLS, async runtime, GPU, SQL, JSON, or native-layout serialization dependencies.
- `andromeda-proto` and `andromeda-catalog` must pass Lot 2 production and dev-dependency allowlists.
- SRPL parser crates must not depend on catalog store implementation crates.
- SRPL Lot 3 crates must pass dev-dependency guards: extracted language-model crates have no dev-dependencies unless an ADR updates the allowlist, while the `andromeda-srpl` facade is limited to the property-test harness.
- Abstract RPC or protocol contract crates must not depend on Quinn runtime crates.

Lot 4.3 additionally guards `andromeda-wal` with a strict allowlist and allows `andromeda-storage` to depend on it as a lower-level durable-kernel owner. Lot 4.4 adds direct owner evidence for `andromeda-wal` through codec/golden/property tests and a direct fuzz target while retaining storage integration evidence for FileWal and recovery. Lot 4.5 requires direct FileWal physical owner evidence in `andromeda-wal` and separate storage integration evidence for API compatibility, ownership reexports, FileWal recovery reports, startup/replay planning, manifest/page integration, and durable visibility boundaries. Expanding those allowlists requires an ADR update or a same-lot governance note that explains why the dependency is C5-safe.

The batch 1 validation gate is:

```bash
cargo test -p andromeda-cli --test workspace_dependency_topology
cargo test -p andromeda-cli --test orphan_source_invariants
...
Lot 4.5 validation commands separate physical owner evidence from storage integration evidence:
