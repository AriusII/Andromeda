...
|---|---|
| `PlanCacheKey` | Must be represented as an explicit typed structure or canonical descriptor. |
| `PlanClass` | Must be represented as an explicit typed structure or canonical descriptor. |
| `ShapeFingerprint` | Must be represented as an explicit typed structure or canonical descriptor. |
| `PlanId` | Must be represented as an explicit typed structure or canonical descriptor. |
| `DecisionTraceId` | Must be represented as an explicit typed structure or canonical descriptor. |

## Invariants

- Key includes ProcedureId, ContractHash, CatalogVersion, StatsVersion, PolicyVersion, PlanClass, and shape fingerprint.
- Plan classes are bounded.
...
- key equality tests.
- plan invalidation tests.
- plan class quota tests.
- hysteresis tests.

## Rejection criteria

- Reject `one plan per arbitrary value`.
- Reject `missing version in key`.
- Reject `unobservable eviction`.
