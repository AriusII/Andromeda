# Specification: ScenarioEvidence v0
...
- Define the purpose and scope of `ScenarioEvidence v0`.
- State the required structures.
- State invariants, errors, security, recovery, tests, and rejection criteria.

## Purpose

Define predictive evidence without making it authoritative.
...
|---|---|
| `ScenarioEvidence` | Must be represented as an explicit typed structure or canonical descriptor. |
| `ConfidenceScore` | Must be represented as an explicit typed structure or canonical descriptor. |
| `CriticalityScore` | Must be represented as an explicit typed structure or canonical descriptor. |
| `FreshnessScore` | Must be represented as an explicit typed structure or canonical descriptor. |
| `RealismScore` | Must be represented as an explicit typed structure or canonical descriptor. |
| `StabilityScore` | Must be represented as an explicit typed structure or canonical descriptor. |
| `ValidationState` | Must be represented as an explicit typed structure or canonical descriptor. |

## Invariants

- ScenarioEvidence is not authoritative.
- Scores remain separate.
- Evidence expires.
- Evidence is tied to versions and workload intent.
...
- expiration tests.
- score separation tests.
- version mismatch tests.

## Rejection criteria

- Reject `single pseudo-truth score`.
- Reject `forced plan by scenario`.
- Reject `evidence without versions`.
