# Specification: ClusterManifest v0
...
- Define the purpose and scope of `ClusterManifest v0`.
- State the required structures.
- State invariants, errors, security, recovery, tests, and rejection criteria.

## Purpose

Define HA/DR cluster manifest, quorum, fencing, and promotion evidence.
...
|---|---|
| `ClusterManifest` | Must be represented as an explicit typed structure or canonical descriptor. |
| `NodeDescriptor` | Must be represented as an explicit typed structure or canonical descriptor. |
| `ClusterEpoch` | Must be represented as an explicit typed structure or canonical descriptor. |
| `FencingToken` | Must be represented as an explicit typed structure or canonical descriptor. |
| `PromotionHistoryHash` | Must be represented as an explicit typed structure or canonical descriptor. |
| `QuorumPolicy` | Must be represented as an explicit typed structure or canonical descriptor. |

## Invariants

- No multi-primary V0.
- Promotion requires quorum and fencing.
- Epoch changes on promotion.
- Nodes reject obsolete primary epoch.
...
- promotion quorum tests.
- obsolete epoch rejection tests.
- fencing tests.
- replica poisoning tests.

## Rejection criteria

- Reject `self-promotion without quorum`.
- Reject `multi-primary V0`.
- Reject `promotion without recovery evidence`.
