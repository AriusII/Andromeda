...
Immediate, incremental, deferred, or snapshot-only Map refresh fails or exceeds budget.
...
- Do not starve WAL.
- Mark Map stale or invalid according to policy.
- Replay MapDeltaLog if safe.
- Rebuild Map from source table if needed.
- Emit Map validation trace.
