...
The expected release evidence command is the command that should appear in the release packet. The workflow coverage note identifies current CI coverage observed in repository workflows; it is not a pass claim.
...
| Workspace check | `cargo check --workspace --all-targets --all-features --locked` | `release-gate-chain.yml` currently runs `cargo check --workspace --locked` in `compile`. Treat that as narrower than the expected release evidence command unless supplemental evidence is attached. | Command transcript, `Cargo.lock` identity, toolchain versions, and full target/feature scope. | Exit code `0` for all workspace targets and all features under the locked dependency graph. |
| Clippy | `cargo clippy --workspace --all-targets --all-features -- -D warnings` | `release-gate-chain.yml` currently runs `cargo clippy --workspace --all-targets --locked -- -D warnings` in `clippy`. Treat missing all-features evidence as residual risk unless supplemental evidence is attached. | Command transcript, lint output, toolchain versions, and any explicit allow-list decision. | Exit code `0`; no warnings under `-D warnings`; no unexplained allow-list expansion. |
| Nextest | `cargo nextest run --workspace --all-features` | No exact nextest gate is visible in the current release-gate workflow. | Nextest output, optional JUnit or archive output when configured, tool version, and test profile details. | Exit code `0`; skipped or flaky tests are recorded as residual risk. |
| Documentation tests | `cargo test --doc --workspace` | No exact documentation-test gate is visible in the current release-gate workflow. | Command transcript, doctest output, toolchain versions, and crate list. | Exit code `0`; failed or ignored doctests are recorded. |
