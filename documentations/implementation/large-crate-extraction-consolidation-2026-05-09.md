...
- `andromeda-quic` depends on abstract protocol/security crates and `andromeda-hadr`. Main incoming crates include `andromeda-cli`, `andromeda-exec`, and `andromeda-quic-runtime-quinn`.
...
| --- | --- | --- |
| `src/manifest.rs`, `src/manifest/format.rs`, `src/manifest/snapshot.rs`, `src/format_version.rs` | `andromeda-manifest` | Keep `manifest/cold_publication.rs` local until placement and publication are decoupled. |
...
| `src/backup/artifact_store/*`, `checkpoint_manager`, `execution_plan`, `scheduler`, `wal_archive_integration` | `andromeda-backup` | DTOs are already mostly owner-backed; move artifact execution and scheduler next. |
...
- Move `StorageFormatFingerprint` and storage-format gate primitives into `andromeda-manifest` if they can be made storage-runtime-free.
...
- Extract backup artifact store, filesystem artifact writer, payload cursors, archive digest, checkpoint manager, execution plan, scheduler, and WAL archive integration into `andromeda-backup`.
...
2. Move manifest and storage format primitives.
...
| `src/procedure_store/*`, `src/procedure_feedback/*` | `andromeda-procedure-store` | Finish moving records, feedback, in-memory store, runtime identity, and tests. |
| `src/statistics/publication/*` | `andromeda-statistics` | Keep catalog activation/version publication in catalog until owner gates pass. |
...
- Move `ProcedureStoreEntry`, invocation decision records, invocation runtime records, feedback records, feedback store, runtime identity, and remaining Procedure Store tests into `andromeda-procedure-store`.
- Move `StatsPublicationSwitch`, publication evidence, publication trace, and publication validation into `andromeda-statistics`; avoid making `statistics` depend strongly on `andromeda-observe` if DTOs can carry trace IDs from `andromeda-observability`.
...
2. Move remaining Procedure Store runtime and feedback.
3. Move statistics publication switch.
...
| Source group | Target crate | Notes |
| --- | --- | --- |
| `src/allocator.rs` | `andromeda-transaction` | Transaction ID allocator belongs with transaction manager. |
| `src/manager.rs`, `src/manager/*` | `andromeda-transaction` | Move manager core, record, lock coordinator, and manager tests. |
| `src/commit_protocol.rs` | `andromeda-transaction` | Keep durable commit protocol with commit log/state. |
...
- Move `TransactionIdAllocator`, `TransactionManager`, `TransactionRecord`, and `TransactionLockCoordinator` to `andromeda-transaction`.
- Add only the minimal owner dependencies needed by `andromeda-transaction`, likely `andromeda-locking`, `andromeda-mvcc`, `andromeda-savepoint`, and `andromeda-transaction-log`.
...
- Move owner tests out of `andromeda-tx`; keep only a short API compatibility smoke test until final facade deletion.
...
1. Move `CommitProtocol`.
2. Move allocator and transaction manager.
3. Move transaction manager tests.
...
- Replace `andromeda-exec -> andromeda-quic` bridge imports with `andromeda-rpc`, `andromeda-rpc-codec`, or `andromeda-rpc-protocol` imports where transport is not required.
...
4. Migrate protocol imports off `andromeda-quic` where only RPC/protocol contracts are needed.
...
| Source group | Target crate | Notes |
| --- | --- | --- |
| `andromeda-proto-wire/src/generated_validation.rs` | Internal modules under `andromeda-proto-wire/src/generated_validation/*` | First split by domain without public behavior change. |
...
| `andromeda-quic/src/backpressure.rs`, `protocol_invariants.rs`, `rpc.rs`, `hadr_streams.rs`, frame/stream/typed-envelope facades in `lib.rs` | `andromeda-rpc-protocol`, `andromeda-rpc-codec`, `andromeda-rpc`, `andromeda-hadr` | Already mostly reexports; migrate callers then delete. |
...
- Split `andromeda-proto-wire/src/generated_validation.rs` into `frame_envelope`, `result_stream`, `invocation`, `catalog_manifest_resolution`, `procedure_manifest`, `structured_object`, `error`, and `views`.
...
- Migrate callers off `andromeda_quic::frame`, `andromeda_quic::stream`, `andromeda_quic::typed_envelope`, and `andromeda_quic::hadr_streams`.
...
1. Split `andromeda-proto-wire::generated_validation` internally.
2. Migrate callers off `andromeda-proto` facades.
3. Migrate callers off `andromeda-quic` RPC/HADR facades.
...
| Source group | Target crate | Notes |
| --- | --- | --- |
| `andromeda-core/src/principal/{permission,surface_scope,contract}.rs` | `andromeda-security-contract` | Runtime-free security vocabulary. |
...
| `andromeda-bench/src/crud/{data,metrics,scenario}.rs` | `andromeda-bench-workload` | Good low-risk advisory extraction. |
| `andromeda-bench/src/runner/{counters,latency,synthetic}.rs` | `andromeda-bench-harness` | Move only if runner does not depend back on `andromeda-bench`. |
...
- Extract runtime-free permission, scope, and principal policy vocabulary from `andromeda-core` into `andromeda-security-contract`.
...
- Move benchmark CRUD workload definitions into `andromeda-bench-workload`.
- Move reusable benchmark counters/latency/synthetic utilities into `andromeda-bench-harness`.
...
2. Extract core security vocabulary to `andromeda-security-contract`.
...
7. Move benchmark workload and harness utilities.
