...
Track the Architecture Decision Record backlog raised by roadmap step 12 for the
Andromeda implementation wave.

This backlog identifies decisions that should be written as ADRs before the
repository treats the related runtime, storage, hardware, or topology behavior
as release-ready doctrine.

## Scope

This document covers six ADR candidates and their current disposition:

- Unsafe Rust policy.
- Binary format and endian policy.
- WAL, commit visibility, and `DurableCommitEvidence`.
- GPU exclusion from critical paths.
- Rust toolchain and MSRV policy, accepted as ADR-0017.
- Macro-engine mapping and crate topology.

The backlog records priority, proposed owner, disposition, source evidence,
acceptance criteria, and residual risk for each candidate. For open items, it
does not approve the decisions. For accepted items, it records the accepted ADR
that owns the decision.

## Non-goals

- Do not edit existing ADR or DEC files.
- Do not allocate final ADR numbers from this backlog alone.
...
1. Pick the highest-priority ADR candidate that matches the current
   implementation work.
2. Draft the ADR in the owning governance location without reusing this backlog
   as final approval.
3. Cite the source evidence listed for the candidate.
4. Include the acceptance criteria and validation commands in the ADR.
5. Update the ADR or DEC index only in a separate owned documentation task.
6. Keep this backlog until each candidate is accepted, rejected, superseded, or
   moved into a more specific roadmap document.

## Backlog

| ID | Candidate ADR | Priority | Proposed owner | Disposition | Source evidence | Acceptance criteria |
|---|---|---:|---|---|---|---|
| ADR-BL-001 | Unsafe Rust policy | P0 | Rust safety and release governance owners | Open | `AGENTS.md` requires `unsafe` to stay private, documented, tested, and reviewed. `DEC-026` makes unsafe drift a doctrine no-go scan. `CURRENT_STATE.md` records broad runtime work but does not make unsafe policy release-specific. | Defines where `unsafe` is allowed, where it is forbidden, required comments, review gates, Miri/fuzz/property expectations, panic/UB escalation, and release-blocking scan behavior. |
| ADR-BL-002 | Binary format and endian policy | P0 | Binary format, storage, WAL, and protocol owners | Open | `AGENTS.md` requires explicit binary codecs and little-endian canonical serialization. `DEC-032` requires storage format identity, golden vectors, unknown-format rejection, and recovery validation before redo. `FrameHeader_RPC_v0.md` records a specific big-endian network-frame exception. | Defines default endian policy, persistent versus network-frame exceptions, version fields, checksums, length bounds, golden vectors, cross-architecture tests, and rejection or migration policy for unsupported bytes. |
| ADR-BL-003 | WAL, commit visibility, and `DurableCommitEvidence` | P0 | Transaction, WAL, storage, recovery, and observability owners | Open | `AGENTS.md`, `ROADMAP_IMPLEMENTATION_2026.md`, and `WORKER_EXECUTION_MATRIX_2026.md` all require no visible commit without durable WAL. Worker 12 records explicit durable LSN evidence for commit and rollback terminal records. `CURRENT_STATE.md` says ProductStock prepared state remains invisible without durable commit evidence. | Defines the canonical evidence type, minimum fields, durability source, ordering rules, rollback evidence, audit/trace projection, recovery replay checks, and tests proving prepared state cannot become visible without durable evidence. |
| ADR-BL-004 | GPU exclusion from critical paths | P0 | Analytics/GPU, hardware, durable-kernel, and topology owners | Open | `AGENTS.md`, `README.md`, `ROADMAP_IMPLEMENTATION_2026.md`, `WORKER_EXECUTION_MATRIX_2026.md`, and `CURRENT_STATE.md` all keep GPU outside commit, rollback, WAL, recovery, MVCC visibility, and security-critical paths. ADR-0011 keeps durable kernel crates free of GPU dependencies. | Defines allowed GPU workloads, forbidden imports and call paths, CPU fallback, kill switches, trace evidence, topology tests, and failure behavior proving GPU absence cannot change contractual or recovered database results. |
...
**Open risk:** Existing documents often state "no unsafe code" or
`forbid(unsafe_code)` at module level, but the repository needs one canonical
workspace policy before SIMD, FFI, or GPU work grows.
...
**Open risk:** `DEC-032` already records candidate format details for B-Tree
keys and heap pages, but a broader ADR is still needed so WAL, catalog,
manifest, and network exceptions do not drift independently.
...
**Open risk:** Worker evidence exists in transaction and execution paths, but
the canonical cross-crate evidence contract is not yet a single ADR.
...
**Open risk:** The roadmap correctly defers GPU runtime, but optional GPU crates
will need a decision before implementation to prevent dependency drift.
...
**Open risk:** ADR-0017 resolves the policy ambiguity, but the current
...
**Open risk:** ADR-0011 is already strong, but a macro-engine map can make the
relationship between product architecture, planes, and Cargo topology easier to
review during later crate splits.

## Validation

This documentation-only backlog was validated with targeted repository
inspection:
...
rg -n "macro-engine|macro engine|macroengine|engine mapping|macro.*mapping|R0|R1|R2|R3|R4|R5|Lot 2|topology|unsafe|little-endian|endian|binary codec|explicit binary|GPU|DurableCommitEvidence|durable commit evidence|commit evidence" documentations docs AGENTS.md README.md -S
rg -n "rust-version|rust-toolchain|MSRV|dependency MSRV|Cargo.lock|lockfile|1\.85|Rust 2024" Cargo.toml rust-toolchain.toml docs documentations AGENTS.md -S
...
change is documentation-only. It adds ADR-0017 and updates this backlog without
editing Rust code, Cargo manifests, `Cargo.lock`, tooling, hooks, skills,
agents, or ADR-0005.
